    ...
    pub fn with_rusb_device_handles(
        ...
        for open_device in device_handles {
            ...
            let mut device = UsbDevice {
                ...
                // Keep the rusb handle so the device-level String descriptor
                // handler can fall back to reading real interface/endpoint
                // strings when the pool misses (needed for UVC webcams etc.).
                rusb_fallback: Some(handle.clone()),
