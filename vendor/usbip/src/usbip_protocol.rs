    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        match *self {
            ...
            Self::UsbIpRetSubmit {
                ...
                // USB/IP RET_SUBMIT wire format:
                //   IN:  48-byte header + transfer_buffer (actual_length bytes) + iso_desc
                //   OUT: 48-byte header only (kernel doesn't read trailing data)
                // `actual_length` is reported in both directions; for OUT the
                // byte count is carried via `transfer_buffer.len()` from the
                // handler but must NOT appear on the wire.
                let is_in = header.direction == Direction::In as u32;
                let wire_transfer: &[u8] = if is_in { transfer_buffer } else { &[] };

                let mut result =
                    Vec::with_capacity(48 + wire_transfer.len() + iso_packet_descriptor.len());
                ...
                result.extend_from_slice(wire_transfer);
                result.extend_from_slice(iso_packet_descriptor);
                result
            }
...
mod tests {
    ...
    fn byte_serialize_usbip_ret_submit_out() {
        // For OUT direction, transfer_buffer MUST NOT appear on the wire
        // (kernel vhci_rx.c only reads trailing bytes for IN). The byte count
        // carried in transfer_buffer (so the caller can compute actual_length)
        // is stripped here. iso_packet_descriptor is still emitted.
        setup_test_logger();
        let res = UsbIpResponse::UsbIpRetSubmit {
            header: UsbIpHeaderBasic {
                command: USBIP_RET_SUBMIT.into(),
            ...
            number_of_packets: 8,
            error_count: 9,
            transfer_buffer: vec![0xFF; 4], // placeholder for byte count — stripped on wire
            iso_packet_descriptor: vec![],
        };

        let bytes = res.to_bytes();
        assert_eq!(bytes.len(), 48, "OUT RET_SUBMIT must be header-only (no trailing data)");
        // actual_length field is at offset 24..28
