...
impl UsbInterfaceHandler for RusbUsbHostInterfaceHandler {
    fn handle_urb(
        ...
        //   - Control: 5s — UVC probe/commit negotiation can exceed 1s.
        //   - Interrupt: 50ms — these are polled continuously; short timeout
        //     lets the mutex cycle to other pending URBs (control/bulk).
        //   - Bulk: 1s — reasonable for most data transfers.
        // A single long timeout across all eps starves concurrent URBs.
        let timeout = match FromPrimitive::from_u8(ep.attributes) {
            Some(EndpointAttributes::Control) => std::time::Duration::from_secs(5),
            Some(EndpointAttributes::Interrupt) => std::time::Duration::from_millis(50),
            Some(EndpointAttributes::Bulk) => std::time::Duration::from_secs(1),
        ...
        if ep.attributes == EndpointAttributes::Control as u8 {
            ...
            if let Direction::In = ep.direction() {
                ...
                // control out — for USB/IP RET_SUBMIT, actual_length must equal
                // the bytes actually written. UVC SET_CUR (probe control) checks
                // this: kernel logs "Failed to set UVC probe control : 0 (exp. 26)"
                // if we report 0 bytes. Return a vec of length = bytes_written;
                // outer writer skips transfer_buffer on wire for OUT direction.
                if let Ok(len) = handle.write_control(
                    setup.request_type,
                    setup.request,
                    setup.value,
                    setup.index,
            ...
            if let Direction::In = ep.direction() {
                ...
                // interrupt out — report bytes sent as actual_length
                if let Ok(len) = handle.write_interrupt(ep.address, req, timeout) {
                    return Ok(vec![0u8; len]);
                }
            }
            ...
            if let Direction::In = ep.direction() {
                ...
                // bulk out — report bytes sent as actual_length
                if let Ok(len) = handle.write_bulk(ep.address, req, timeout) {
                    return Ok(vec![0u8; len]);
                }
            }
        }
...
impl UsbDeviceHandler for RusbUsbHostDeviceHandler {
    fn handle_urb(
        ...
        if setup.request_type & 0x80 == 0 {
            // control out — return bytes_written as the length so the outer
            // RET_SUBMIT reports correct actual_length (see interface handler).
            if let Ok(len) = handle.write_control(
                setup.request_type,
                setup.request,
                setup.value,
