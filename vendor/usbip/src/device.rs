...
pub struct UsbDevice {
    ...
    /// Optional rusb handle for fallback string descriptor reads.
    /// Real composite devices (UVC webcams, audio devices, etc.) embed
    /// interface/endpoint string indices in their descriptors that the
    /// string pool doesn't know about; when the client asks for one we
    /// read it live from the host device.
    #[cfg_attr(feature = "serde", serde(skip))]
    pub(crate) rusb_fallback:
        Option<Arc<Mutex<rusb::DeviceHandle<rusb::GlobalContext>>>>,
}
                        ...
                        match FromPrimitive::from_u16(setup_packet.value >> 8) {
                            ...
                            Some(String) => {
                                ...
                                if index == 0 {
                                    ...
                                    // Try the pool first, then fall back to
                                    // reading the real device's string (rusb
                                    // path). Composite devices like UVC
                                    // webcams reference interface string
                                    // indices that aren't in the pool.
                                    let s: Option<std::string::String> = self
                                        .string_pool
                                        .get(&index)
                                        .cloned()
                                        .or_else(|| {
                                            self.rusb_fallback.as_ref().and_then(|h| {
                                                let handle = h.lock().ok()?;
                                                handle
